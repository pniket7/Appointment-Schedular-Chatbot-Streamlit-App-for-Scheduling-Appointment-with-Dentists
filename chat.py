...
def main():
    ...
    # Check if the sessionAdvisor object exists in session_state
    ...
    advisor_responses = [message['content'] for message in chat_history] if chat_history else []

    # Display the chat history with text wrapping
    for response in advisor_responses:
        st.markdown(f'**Advisor:** {response}', unsafe_allow_html=True)
